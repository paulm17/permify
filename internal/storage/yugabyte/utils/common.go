...
func SnapshotQuery(sl squirrel.SelectBuilder, value uint64) squirrel.SelectBuilder {
	...
	valStr := fmt.Sprintf("'%v'::bigint", value)
	...
	visibilityExpr := squirrel.Expr(fmt.Sprintf("txid_visible_in_snapshot(created_tx_id, %s) = true", snapshotQuery))
	...
	expiredVisibilityExpr := squirrel.Expr(fmt.Sprintf("txid_visible_in_snapshot(expired_tx_id, %s) = false", snapshotQuery))
	// Create an expression to check if the expired_tx_id is equal to zero.
	expiredZeroExpr := squirrel.Expr("expired_tx_id = '0'::bigint")
...
func snapshotQuery(value uint64) (string, string) {
	...
	visibilityExpr := fmt.Sprintf("txid_visible_in_snapshot(created_tx_id, %s) = true", snapshotQ)
	...
	expiredVisibilityExpr := fmt.Sprintf("txid_visible_in_snapshot(expired_tx_id, %s) = false", snapshotQ)
	// Create an expression that checks if the 'expired_tx_id' is zero. This handles cases where the transaction hasn't expired.
	expiredZeroExpr := "expired_tx_id = '0'::bigint"
